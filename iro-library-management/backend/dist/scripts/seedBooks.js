...
const sampleBooks = [
    ...
    },
    {
        ...
        language: "English",
        pages: 298,
        description: "Examining contemporary issues in Islamic thought and their relevance to modern society.",
        categories: [
            "Islamic Philosophy",
            "Contemporary Issues",
            "Research & Reference",
...
const seedDatabase = async () => {
    try {
        ...
        const adminUser = await User_1.default.findOne({ email: "admin@iro.com" });
        let adminId;
        if (!adminUser) {
        ...
        const superAdminUser = await User_1.default.findOne({ email: "superadmin@iro.com" });
        if (!superAdminUser) {
        ...
        const createdBooks = await Book_1.default.insertMany(booksWithMetadata);
